import { config, ContextTags, extensionPoint, extensions, Getter, Provider } from '@loopback/core'
...
export class MultiTenancyMiddlewareProvider implements Provider<Middleware> {
  ...
  async action(requestCtx: RequestContext) {
    ...
    debug('Tenant identified by strategy %s', tenancy.strategy.name, tenancy.tenant)
